...
if submit is True:
    ...
    if var==1:
        ...
        for i in range(0,int(numberOfVideos)):
            ...
            fin = yt.streams.filter(progressive=True, file_extension='mp4').order_by('resolution')[-1].download() # Generate a unique filename using a timestamp
            unique_id = uuid.uuid4().hex[:6]  # Get the first 6 characters of a UUID as a unique identifier
            new_filename = f'Video-{i}-{unique_id}.mp4'

            os.rename(fin, new_filename)
            vidfiles.append(new_filename)
